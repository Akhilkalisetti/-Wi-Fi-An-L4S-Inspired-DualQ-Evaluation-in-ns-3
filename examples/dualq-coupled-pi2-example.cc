...
{
  uint32_t qSize = StaticCast<DualQCoupledPi2QueueDisc> (queue)->GetQueueSize ();
...
{
  LogComponentEnable ("DualQCoupledPi2QueueDisc", LOG_LEVEL_INFO);
  ...
  Config::SetDefault ("ns3::DualQCoupledPi2QueueDisc::Mtu", UintegerValue (meanPktSize));
  ...
  TrafficControlHelper tchDualQCoupledPi2;
  handle = tchDualQCoupledPi2.SetRootQueueDisc ("ns3::DualQCoupledPi2QueueDisc");
  tchDualQCoupledPi2.AddInternalQueues (handle, 2, "ns3::DropTailQueue", "MaxSize", StringValue ("100000B"));
  ...
  queueDiscs = tchDualQCoupledPi2.Install (devn2n3);
  ...
  if (printDualQCoupledPi2Stats)
    {
      ...
      std::cout << "Dropped packets: "
                << st.GetNDroppedPackets (DualQCoupledPi2QueueDisc::FORCED_DROP)
                << std::endl;
