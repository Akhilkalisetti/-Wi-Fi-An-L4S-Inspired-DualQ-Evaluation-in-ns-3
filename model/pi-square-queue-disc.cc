...
PiSquareQueueDisc::CheckConfig (void)
{
  ...
  if (GetNInternalQueues () == 0)
    {
      ...
      if (m_mode == QUEUE_DISC_MODE_PACKETS)
        {
          QueueSize queueSize (PACKETS, m_queueLimit);
  ...
  if ((GetInternalQueue (0)->GetMaxSize ().GetUnit () == PACKETS && m_mode == QUEUE_DISC_MODE_BYTES)
      || (GetInternalQueue (0)->GetMaxSize ().GetUnit () == BYTES && m_mode == QUEUE_DISC_MODE_PACKETS))
     {
       NS_LOG_ERROR ("The mode of the provided queue does not match the mode set on the PiSquareQueueDisc");
       return false;
     }

  if (GetInternalQueue (0)->GetMaxSize ().GetValue () < m_queueLimit)
    {
